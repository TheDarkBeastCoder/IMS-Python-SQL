...
def purchase_product(role, username):
    try:
        ...
        WHERE Product_ID=%s
        """
        execute_query(update_query, (new_stock, pid))
        insert_query = """
        INSERT INTO purchases
        (Product_ID, Product_Name, Quantity_Purchased,
        Total_Price, Purchased_By)
        VALUES(%s, %s, %s, %s, %s)
        """
        execute_query(insert_query, (pid,pname,qty_purchased,total_price,username))
        print("\n========== PURCHASE BILL ==========")
...
def purchase_history(role, username):
    if role in ['admin', 'staff']:
        ...
        SELECT *
        FROM purchases
        WHERE Purchased_By=%s
        """
        records = fetch_all(query, (username,))
    if not records:
